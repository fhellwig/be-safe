...
function list(res) {
    ...
    async.series([
        ...
        function (err) {
            ...
            db.close();
            res.send(process.env.be_safe_host_name || config.be_safe_host_name + ":" + config.server.port);
        });
}
...
function init(file) {
    ...
    var db = new sqlite3.Database(file);
    db.run("create table if not exists subscriptions (id integer primary key autoincrement,email text not null,query text not null,uuid test,sub_type text not null,createdat timestamp default CURRENT_TIMESTAMP)");
    db.close();
}

function unsubscribe(req, res) {

    var targetID = req.params.subscription_id ? req.params.subscription_id : null;
    var uuid = req.params.uuid ? req.params.uuid : null;

    if (targetID) {
        var db = new sqlite3.Database(file);
        var stmt = db.prepare("delete from subscriptions where id = (?) and uuid = (?2)");
        stmt.run({
            1: targetID,
            2: uuid
        }, function (err) {
            if (err) {
                res.send({
                    msg: "There was an error unsubscribing.",
                    status: 403
                });
            } else {
                res.send({
...
function subscribe(email, query, callback) {
    ...
    async.series([
            function (cb) {
                db.get("select count(email) as count, query from subscriptions where email=? and sub_type=?2", {
                    1: inputEmail,
                    2: subType
                }, function (err, row) {
                    if (err) return cb(err);
            ...
            function (cb) {
                if (recordCount == 1) {
                    db.run("update subscriptions set query = ? where email = ?2", {
                        2: inputEmail,
                        1: query_for_db
                    }, function (err) {
                        if (err) return cb(err);
                    ...
                    db.run("INSERT INTO subscriptions (email, query, uuid, sub_type) VALUES (?,?4,?2,?3)", {
                        1: inputEmail,
                        2: UUID,
                        3: subType,
                        4: query_for_db
...
function send(template_info, to) {
    ...
    aws.config = {
            ...
            "secretAccessKey": process.env.secretAccessKey || config.aws.secretAccessKey,
            "accessKeyId": process.env.accessKeyId || config.aws.accessKeyId
        }
        // load AWS SES
    var ses = new aws.SES({
...
function add_subscribe_template_info(data, cb) {
    ...
    template.text = template.text.replace(/\[{2}subscribe type\]{2}/g, toWordCase(data.subscribe_type));
    template.html = template.html.replace(/\[{2}subscribe type\]{2}/g, toWordCase(data.subscribe_type));

    template.text = template.text.replace(/\[{2}brand name\]{2}/g, data.orig_query.brand_name);
    template.html = template.html.replace(/\[{2}brand name\]{2}/g, data.orig_query.brand_name);

    template = add_footer(template, data);

    //add the specific subscription type to the subject
    template.subject = "You've been subscribed to a BE Safe " + toWordCase(data.subscribe_type) + " Alert!",
...
function add_footer(template, data) {
    ...
    template.text = template.text.replace("[[unsublink]]", link + "/api/subscribe/remove/" + data.insertID + "/" + data.uuid);
    template.html = template.html.replace("[[unsublink]]", link + "/api/subscribe/remove/" + data.insertID + "/" + data.uuid);
...
function add_example_template_info(data, cb) {
    ...
    drugs.search(data.orig_query, function (response) {
        if (response.status !== 'success') {
            ...
            template.text += config.emails.example[data.subscribe_type].brand.text.replace("[[brand name]]", data.orig_query.brand_name);
            template.html += config.emails.example[data.subscribe_type].brand.html.replace("[[brand name]]", data.orig_query.brand_name);
            var max = (db_results.length > 3) ? 3 : db_results.length;

            if (data.subscribe_type == "recalls") {
                for (i = 0; i < max; i++) {
                    template.text += config.emails.example[data.subscribe_type].row.text.replace("[[recall reason]]", db_results[i].reason);
                    template.html += config.emails.example[data.subscribe_type].row.html.replace("[[recall reason]]", db_results[i].reason);
                }
            } else {
                for (i = 0; i < max; i++) {
            ...
            template = add_footer(template, data);
            //add the specific subscription type to the subject
            template.subject = "BE Safe " + toWordCase(data.subscribe_type) + " Summary";
            cb(template);
        }
    });
}
