...
function subscribe(email, query, unsubscribeLink, callback) {
    ...
    async.series([
            ...
            function (cb) {
                if (recordCount == 1) {
                    db.run("update subscriptions set query = ? where email = ?2 and sub_type=?3", {
                        3: subType,
                        2: inputEmail,
                        1: query_for_db
                    }, function (err) {
...
function add_footer(template, data) {
    ...
    var link = data.unsubscribeLink.replace(data.unsubscribeLink.substring(data.unsubscribeLink.indexOf("#")-1),'');
    link = link + "/api/subscribe/remove/" + data.insertID + "/" + data.uuid;
