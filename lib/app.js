...
// We are local if our hostname is localhost or if the PORT environment
// variable is not set (or is the same as our config value) as it would
// be if we are/ running on Azure or AWS that set the port to a pipe.
function isLocal(req) {
  return req.hostname === 'localhost' || !process.env.PORT ||
    parseInt(process.env.PORT) === config.server.port;
}
